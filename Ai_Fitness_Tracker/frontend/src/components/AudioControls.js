...
const AudioControls = ({
  ...
  return (
    <div className="flex flex-row gap-3 p-4 animate-in fade-in duration-500">
      {/* Audio Cues Toggle */}
      <button
        className={`flex flex-row items-center gap-2 py-2.5 px-4 rounded-2xl bg-white/5 border-2 transition-all ${
      ...
      >
        ...
        <span className={`text-[13px] font-bold ${
          audioCuesEnabled ? 'text-primary' : 'text-zinc-500'
        }`}>
          Audio Cues
        </span>
      </button>
      ...
      >
        ...
        <span className={`text-[13px] font-bold ${
          voiceCoachEnabled ? 'text-primary' : 'text-zinc-500'
        }`}>
          Voice Coach
        </span>
      </button>
